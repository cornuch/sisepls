...
#' @param data : data frame. The data source containing all the variables specified in the formula. 'NA's are not allowed.
#' @param n_components : integer, default=2. Number of components to keep. Should be between 1 and the max(n_features).
#' @param parallele : bool, default=TRUE.
#'
#' @import parallel
#' @import doParallel
#' @import foreach
...
fit_pls_parall<-function(formule,data,n_components=2,parallele=TRUE){
  ...
  if (parallele==TRUE){
    ...
    max_cores<-parallel::detectCores(logical=TRUE)-1

    #number of core used
    if (ncol(Y_dummy)<=max_cores){
      n_cores<-ncol(Y_dummy)
    }else{
      #??
      n_cores<-1
    }

    args<-expand.grid(modalite = c(1:n))

    #Open cores
     cl <- parallel::makeCluster(n_cores)
     doParallel::registerDoParallel(cl)

     #Computing fit one simple in paralell for each outcome
     clusterExport(cl, c("args"))
     objet_pls<-foreach(i = 1:n,.inorder=TRUE) %dopar% {
       return(fit_one_simple(f,X,Y,Y_dummy,X0,Y0,modalite=args$modalite[i],n_components))
     }
